...
##############################################################################################################
    ...
    def accept_new_connection(self, io_device):
        ...
        self.sel.register(conn, selectors.EVENT_READ | selectors.EVENT_WRITE, data=BaseConnectionData())
    ...
    def handle_io_device_events(self, io_device, event_mask):
        ...
        if event_mask & selectors.EVENT_READ:
            recv_data = io_device.fileobj.recv(2048)
            if recv_data:
...
##############################################################################################################
    ...
    def send_message_to_host(self, destination_id, message):
        ...
        if destination_id in self.hosts_db:
            ...
            # TODO: Implement the above functionality
            first_link = self.hosts_db[destination_id].first_link_id

            for key in self.sel.get_map().values():
                if key.data and hasattr(key.data, 'id') and key.data.id == first_link:
    ...
    def broadcast_message_to_servers(self, message, ignore_host_id=None):
        ...
        for server_id in self.adjacent_server_ids:
            if server_id != ignore_host_id:
                # Find the socket for this adjacent server
                for key in self.sel.get_map().values():
                    if key.data and hasattr(key.data, 'id') and key.data.id == server_id:
                        key.data.write_buffer += message
                        break
    ...
    def broadcast_message_to_adjacent_clients(self, message, ignore_host_id=None):
        ...
        for client_id in self.adjacent_user_ids:
            if client_id != ignore_host_id:
                # Find the socket for this adjacent client
                for key in self.sel.get_map().values():
                    if key.data and hasattr(key.data, 'id') and key.data.id == client_id:
                        key.data.write_buffer += message
                        break
...
##############################################################################################################
    ...
    def handle_server_registration_message(self, io_device, message):
        ...
        Returns:
            None        
        """
        # i used ai to help me find the error in this function that was preventing certain tests from passing
        # TODO: Implement the functionality described above
        if message.source_id in self.hosts_db:
            error_msg = "A machine has already registered with ID %d" % message.source_id
            status = StatusUpdateMessage.bytes(self.id, 0, 0x02, error_msg)
        ...
        if is_adjacent:
            first_link_id = message.source_id
        else:
            first_link_id = message.last_hop_id

        # Create ServerConnectionData
        ...
        # If adjacent, update the io_device and adjacent_server_ids list
        if is_adjacent:
            ...
            my_reg_msg = ServerRegistrationMessage.bytes(
                self.id, 0, self.server_name, self.server_info
            )
            server_data.write_buffer += my_reg_msg

            # send info abt all other existing servers / clients to new server
            for host_id, host_data in self.hosts_db.items():
                ...
                if isinstance(host_data, ServerConnectionData):
                    reg_msg = ServerRegistrationMessage.bytes(
                        host_data.id, self.id, host_data.server_name, host_data.server_info
                    )
                    server_data.write_buffer += reg_msg
                elif isinstance(host_data, ClientConnectionData):
                    reg_msg = ClientRegistrationMessage.bytes(
                        host_data.id, self.id, host_data.client_name, host_data.client_info
                    )
                    server_data.write_buffer += reg_msg

        # Broadcast new server to all other adj servers
        broadcast_msg = ServerRegistrationMessage.bytes(
            message.source_id, self.id, message.server_name, message.server_info
        )

        if is_adjacent:
            # Don't send back to  new server
...
##############################################################################################################
    ...
    def handle_client_registration_message(self, io_device, message):
        ...
        # TODO: Implement the above functionality
        if message.source_id in self.hosts_db:
            error_msg = "Someone has already registered with ID %d" % message.source_id
            status = StatusUpdateMessage.bytes(self.id, 0, 0x02, error_msg)
            self.send_message_to_unknown_io_device(io_device, status)
            return
        ...
        # Create ClientConnectionData and store in hosts_db
        ...
        if is_adjacent:
            ...
            for host_id, host_data in self.hosts_db.items():
                if host_id == message.source_id:
                    continue  # Don't send info about itself

                # send client registration messages to client
        ...
        # Broadcast new client to all adjacent servers / clients
        broadcast_msg = ClientRegistrationMessage.bytes(
            message.source_id, self.id, message.client_name, message.client_info
        )
...
##############################################################################################################
    ...
    def handle_client_chat_message(self, io_device, message):
        ...
        if message.destination_id in self.hosts_db:
            # Forward the chat message to its destination
            self.send_message_to_host(message.destination_id, message.bytes)
        else:
            # Destination doesn't exist - send error back to sender
            error_msg = "Unknown ID %d" % message.destination_id
            status = StatusUpdateMessage.bytes(self.id, message.source_id, 0x01, error_msg)
            self.send_message_to_host(message.source_id, status)
...
##############################################################################################################
    ...
    def handle_client_quit_message(self, io_device, message):
        ...
        if message.source_id not in self.hosts_db:
            ...
            # Broadcast the quit message to all adjacent servers and clients (except the quitting client)
        ...
        # Remove the client from hosts_db
        del self.hosts_db[message.source_id]

        # If this client was adjacent, remove from adjacent_user_ids list
        if message.source_id in self.adjacent_user_ids:
            self.adjacent_user_ids.remove(message.source_id)
            # Also unregister and close the socket
            self.sel.unregister(io_device.fileobj)
            io_device.fileobj.close()
